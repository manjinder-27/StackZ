    ...
    try:
        ...
        if last_progress is None: #User has no progress yet
            if module_completed_index != 0: #But trying to update other than first module
                return Response({'detail':'Invalid/Tampered Request'},status=HTTP_400_BAD_REQUEST)
            else:
                progress = UserProgress(user=user,course_id=course_id,module_index=0)
                progress.save()
                return Response({'detail':'Progress Updated'},status=HTTP_204_NO_CONTENT)
...
def get_course_progress(request):
    try:
        ...
        return Response({'detail':'No Progress data available'},status=HTTP_404_NOT_FOUND)
    except:
        return Response({'detail':'Invalid Request'},status=HTTP_400_BAD_REQUEST)
